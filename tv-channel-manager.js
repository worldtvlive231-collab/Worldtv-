...
function registerRoutes(app){
  ...
  app.post("/api/admin/tv-channels", adminOnly, (req,res)=>{
    ...
    try{
      const info = tvDb.prepare(`
        INSERT INTO tv_channels(name,logo_url,country,active) VALUES(?,?,?,?)
      `).run(name,logoUrl,country,active);
  ...
  app.put("/api/admin/tv-channels/:id", adminOnly, (req,res)=>{
    ...
    try{
      tvDb.prepare(`
        UPDATE tv_channels SET name=?,logo_url=?,country=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=?
      `).run(name,logoUrl,country,active,old.id);
  ...
  app.delete("/api/admin/tv-channels/:id", adminOnly, (req,res)=>{
    const channel = tvDb.prepare("SELECT id FROM tv_channels WHERE id=?").get(req.params.id);
  ...
  app.post("/api/admin/match-tv-channels", adminOnly, (req,res)=>{
    ...
    const home = clean(req.body?.home_team,160);
    const away = clean(req.body?.away_team,160);
    ...
    tvDb.prepare(`
      ...
      VALUES(?,?,?,?,?,?,?,?,1)
      ON CONFLICT(match_key,channel_id) DO UPDATE SET
    ...
    const row = tvDb.prepare(`
      SELECT a.*,c.name AS channel_name,c.logo_url AS channel_logo,c.country AS channel_country
      FROM match_tv_channels a JOIN tv_channels c ON c.id=a.channel_id
      WHERE a.match_key=? AND a.channel_id=?
    `).get(matchKey,channelId);
    res.json(row);
  });
...
express.application.listen = function patchedListen(...args){
  if(!this.__tvChannelManagerInstalled){
    ...
    if(router && Array.isArray(router.stack)){
      ...
      // server.js ends with one 404 middleware. Put these API routes just before it.

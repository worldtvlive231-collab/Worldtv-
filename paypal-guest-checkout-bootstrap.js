  ...
  global.fetch=async function worldTvPayPalGuestFetch(input,options={}){
    try{
      ...
      if(method==="POST" && /^https:\/\/api-m(?:\.sandbox)?\.paypal\.com\/v2\/checkout\/orders(?:\?.*)?$/.test(url) && typeof options.body==="string"){
        ...
        if(context){context.landing_page="GUEST_CHECKOUT";options={...options,body:JSON.stringify(body)};}
      }
    }catch(error){console.warn("WORLD TV PayPal guest-checkout preference could not be applied:",error&&error.message||error);}
    return originalFetch(input,options);
  };
}

const GUEST_SCRIPT="/assets/paypal-guest-checkout.js?v=20260824-3";
...
function serveInjected(filePath,scripts,res,next){
  fs.readFile(filePath,"utf8",(err,html)=>{
    if(err){if(typeof next==="function")return next();return res.status(500).send("Page unavailable");}
...
express.application.get=function worldTvGuestGet(routePath,...handlers){
  if(typeof routePath==="string"&&routePath.toLowerCase()==="/reseller")return this.route(routePath).get((req,res,next)=>serveInjected(path.join(process.cwd(),"reseller.html"),[RESELLER_SCRIPT,GUEST_SCRIPT],res,next));
...
async function createGuestCheckout(req,res){
  try{
    ...
    if(!user){const inserted=db.prepare("INSERT INTO users(name,email,password_hash) VALUES(?,?,NULL)").run(name,email);user={id:Number(inserted.lastInsertRowid),name,email};}
    else if(!String(user.name||"").trim()&&name)db.prepare("UPDATE users SET name=? WHERE id=?").run(name,user.id);
    db.prepare(`INSERT INTO checkout_requests(reference,user_id,plan_id,coupon_code,original_amount_usd,discount_usd,final_amount_usd,fx_ghs_per_usd,original_amount_ghs,discount_ghs,final_amount_ghs) VALUES(?,?,?,?,?,?,?,?,?,?,?)`).run(reference,user.id,plan.id,couponCode||null,originalUsd,Number(discountUsd.toFixed(2)),finalUsd,Number(ghsPerUsd.toFixed(6)),originalGhs,discountGhs,finalGhs);
    res.json({ok:true,reference,plan:plan.name,final_amount_usd:finalUsd,email,guest:true});
  }catch(error){console.error("Guest subscription checkout error:",error);res.status(500).json({error:"Could not start guest checkout. Please try again."});}
}
...
express.application.listen=function worldTvGuestListen(...args){
  if(!this.__worldTvGuestSubscriptionRouteInstalled){
    ...
    const router=this._router;const before=router&&Array.isArray(router.stack)?router.stack.length:0;
    this.post("/api/guest/checkout-request",createGuestCheckout);
    if(router&&Array.isArray(router.stack)){const added=router.stack.splice(before);const insertAt=Math.max(0,router.stack.length-2);router.stack.splice(insertAt,0,...added);}
  }

...
app.get("/api/test-reseller", (req,res)=>res.json({status: "reseller endpoint working"}));

app.get("/api/admin/resellers", adminOnly, (req,res)=>res.json(db.prepare("SELECT r.id,r.name,r.email,r.phone,r.commission_percent,r.status,r.created_at,COUNT(DISTINCT rs.id) as total_sales,COALESCE(SUM(rs.commission_ghs), 0) as total_commissions,COALESCE(SUM(CASE WHEN rp.status='pending' THEN rp.amount_ghs ELSE 0 END), 0) as pending_payout FROM resellers r LEFT JOIN reseller_sales rs ON rs.reseller_id = r.id LEFT JOIN reseller_payouts rp ON rp.reseller_id = r.id GROUP BY r.id ORDER BY r.created_at DESC").all()));

app.post("/api/admin/resellers", adminOnly, async (req,res)=>{const {name, email, phone, commission_percent, password} = req.body || {};if(!name || !email || !commission_percent || !password) return res.status(400).json({error: "Name, email, commission, and password required"});try {const hash = await bcrypt.hash(String(password), 12);const result = db.prepare("INSERT INTO resellers(name, email, phone, commission_percent, password_hash, status) VALUES(?, ?, ?, ?, ?, 'active')").run(String(name).trim(), String(email).trim().toLowerCase(), String(phone || '').trim(), Number(commission_percent), hash);audit("reseller_created", "reseller", result.lastInsertRowid, `${name} - ${email}`);res.json({ok: true, id: result.lastInsertRowid});} catch(e) {res.status(500).json({error: e.message});}});

 ...
 * Supports GHS-priced products and USD-priced worldwide subscriptions.
...
const CurrencyConverter={
  ...
  exchangeRates:{GHS:1},lastUpdated:null,activeCurrency:'USD',baseSubscriptionPriceUsd:23,
  ...
  convertUsd(usd,currency){const a=Number(usd);if(!Number.isFinite(a))return null;if(!currency||currency==='USD')return a;const usdRate=Number(this.exchangeRates.USD);const targetRate=Number(this.exchangeRates[currency]);return Number.isFinite(usdRate)&&usdRate>0&&Number.isFinite(targetRate)&&targetRate>0?a*targetRate/usdRate:null;},
  ...
  getExchangeRateDisplay(currency){if(currency==='GHS')return'Prices shown in Ghana cedis';const rate=Number(this.exchangeRates[currency]);if(!Number.isFinite(rate)||rate<=0)return'Exchange rate unavailable';const ghsPerUnit=1/rate;const digits=ghsPerUnit>=100?0:ghsPerUnit>=10?2:4;return`1 ${currency} ≈ GH₵${new Intl.NumberFormat('en-US',{minimumFractionDigits:digits,maximumFractionDigits:digits}).format(ghsPerUnit)}`;},
  updateAllPrices(currency){
    if(!this.isSupportedCurrency(currency))currency='USD';
    this.activeCurrency=currency;
    document.querySelectorAll('[data-price-usd]').forEach(el=>{
      const usd=Number(el.getAttribute('data-price-usd'));
      if(!Number.isFinite(usd))return;
      const local=this.convertUsd(usd,currency);
      const localLine=currency!=="USD"&&local!==null
        ? `<br><span class="currency-converted">≈ ${this.format(local,currency)} ${currency} at checkout rates</span>`
        : "";
      el.innerHTML=`<strong>${this.format(usd,'USD')} USD</strong>${localLine}`;
    });
    document.querySelectorAll('[data-price-ghs]').forEach(el=>{
      const g=Number(el.getAttribute('data-price-ghs'));
      if(!Number.isFinite(g))return;
      if(currency==='GHS'){el.innerHTML=`<strong>${this.format(g,'GHS')}</strong>`;return;}
      const c=this.convert(g,currency);
      el.innerHTML=c===null
        ? `<strong>${this.format(g,'GHS')}</strong><br><span class="currency-converted">Conversion unavailable</span>`
        : `<strong>${this.format(c,currency)} ${currency}</strong><br><span class="currency-converted">≈ ${this.format(g,'GHS')}</span>`;
    });
  },
  ...
  updateTopRightCurrencyBar(currency){const bar=document.getElementById('top-right-currency-bar');if(!bar)return;if(!this.isSupportedCurrency(currency))currency='GHS';const info=this.currencies[currency];bar.innerHTML=`<div style="display:flex;align-items:center;gap:6px;font-weight:800;font-size:14px"><span>${info.flag}</span><span>${currency}</span></div><div style="text-align:right;font-size:11px;color:#716958">${this.getExchangeRateDisplay(currency)}</div>`;bar.style.display='flex';},

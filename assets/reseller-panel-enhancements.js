(()=>{
  ...
  async function refreshStore(){
    ...
    try{
      ...
      const storeRes=await fetch('/api/reseller/code-store',{headers:{'x-reseller-token':token}});
      ...
      if($('storeStatus')){$('storeStatus').className='msg success';$('storeStatus').textContent=`Reseller price is ${usd(d.unit_price_usd||UNIT_USD)} per 1-year code. Minimum purchase: ${MIN_CODES} codes (${usd(MIN_CODES*(d.unit_price_usd||UNIT_USD))}). Pay securely with Paystack. Paid code credits are added only after successful payment verification.`;}
      if($('buyCodesBtn')){$('buyCodesBtn').disabled=!d.payment_configured;$('buyCodesBtn').textContent='Pay with Paystack';}
      ensurePaymentArtwork();removePaypalUi();
  ...
  function init(){
    ...
    refreshStore();setTimeout(refreshStore,800);
  }

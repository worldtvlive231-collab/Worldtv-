...
function showComplete(email,url){
 const panel=document.getElementById('downloadComplete');
 if(!panel)return;
 panel.hidden=false;
 const emailEl=document.getElementById('downloadCompleteEmail');
 const manual=document.getElementById('manualApkLink');
 if(emailEl)emailEl.textContent=email||'';
 if(manual)manual.href=url||APK_URL;
 panel.scrollIntoView({behavior:'smooth',block:'start'});
}
function startDownload(url){
 const finalUrl=url||APK_URL;
 let frame=document.getElementById('wtvApkDownloadFrame');
 if(!frame){
  frame=document.createElement('iframe');
  frame.id='wtvApkDownloadFrame';
  frame.title='WORLD TV APK download';
  frame.style.display='none';
  document.body.appendChild(frame);
 }
 frame.src=finalUrl+(finalUrl.includes('?')?'&':'?')+'download_ts='+Date.now();
}
async function submit(e){
 ...
 try{
  ...
  const url=d.download_url||APK_URL;
  msg.style.color='#1e6b34';msg.textContent='Thank you! Your download is starting and your email guide is being sent.';
  setTimeout(()=>{
   if(modal)modal.style.display='none';
   showComplete(email,url);
   startDownload(url);
  },450);

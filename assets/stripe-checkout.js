(()=>{
  ...
  function addStyles(){
    ...
    style.id=STYLE_ID;
    style.textContent=`
      .wtv-stripe-wrap{width:100%;margin-top:12px;text-align:center}
  ...
  function setPageWording(){
    document.querySelectorAll("p.muted.center").forEach(p=>{
      if(/choose paystack or paypal/i.test(p.textContent||""))p.textContent="Choose Paystack, PayPal or Stripe to complete your subscription securely.";
    });
    document.querySelectorAll(".payment-note").forEach(note=>{
      if(note.dataset.stripeUpdated)return;
      note.dataset.stripeUpdated="1";
      const text=note.textContent||"";
      if(/PayPal charges/i.test(text))note.innerHTML='<b>Worldwide pricing:</b> PayPal and Stripe charge the US$23 base price directly. Paystack charges the current Ghana-cedi equivalent. Any valid coupon is applied before payment.';
    });
  }
  ...
  async function createAccount(form){
    const msg=form.querySelector(".wtv-stripe-signup-msg");
    ...
    try{
      let referral_code="";
      try{referral_code=String(window.WorldTVMarketing?.getAttribution?.()?.referral_code||"").trim().toUpperCase();}catch(_){ }
      ...
      if(!response.ok){
        if(String(data.error||"").toLowerCase().includes("already")){msg.innerHTML='This email already has a WORLD TV account. <a href="/login.html?next=%2Fsubscribe.html">Log in here</a> and return to pay.';return false;}
      ...
      msg.textContent="Account created. Opening secure Stripe checkout...";
      return true;
  ...
  async function startStripe(){
    ...
    working=true;
    const btn=document.querySelector(".wtv-stripe-btn");
    ...
    try{
      ...
      if(!data.checkout_url)throw new Error("Stripe checkout URL was not received.");
  ...
  function createUi(){
    ...
    wrap.id=WRAP_ID;wrap.className="wtv-stripe-wrap";wrap.style.display="none";
    ...
    const guestWrap=paypalBtn.parentElement?.querySelector(":scope > .wtv-paypal-guest-wrap");
    (guestWrap||paypalBtn).insertAdjacentElement("afterend",wrap);
    ...
    button.onclick=()=>{
      ...
      if(token){startStripe();return;}
      form.classList.toggle("show");if(form.classList.contains("show"))form.querySelector('[name="name"]')?.focus();
    };
  ...
  async function loadConfig(){
    try{
      ...
      const wrap=document.getElementById(WRAP_ID);if(wrap)wrap.style.display=configured?"block":"none";
  ...
  async function handleReturn(){
    ...
    }catch(error){showMessage(`<div class="error">${String(error.message||"Could not confirm Stripe payment.")}</div>`);}
  }

  function scan(){createUi();const paypalBtn=document.getElementById("paypalBtn"),wrap=document.getElementById(WRAP_ID);if(paypalBtn&&wrap){const guestWrap=paypalBtn.parentElement?.querySelector(":scope > .wtv-paypal-guest-wrap");if(guestWrap&&wrap.previousElementSibling!==guestWrap)guestWrap.insertAdjacentElement("afterend",wrap);}setPageWording();}
